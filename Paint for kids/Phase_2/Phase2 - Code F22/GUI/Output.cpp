...
Output::Output()
{
	...
	UI.isFillChanged = false;	//Set fill color not changed at the beginning
...
bool Output::getstate(int& x, int& y)
{
...
void Output::setmouse(int& x, int& y)
{
...
void Output::CreateDrawToolBar() const
{
	...
	MenuItemImages[ITM_SOUND] = "images\\MenuItems\\no-sound.jpg";

	if (sound != Initial && UI.InterfaceMode == MODE_DRAW)
	{
		if (sound == Enable)
		{
			MenuItemImages[ITM_SOUND] = "images\\MenuItems\\music.jpg";
			pWind->DrawImage(MenuItemImages[ITM_SOUND], ITM_SOUND * UI.MenuItemWidth, 0, UI.MenuItemWidth, UI.ToolBarHeight);
			sound = Disable;
		}
		else
		{
...
bool Output::Sound() const
{
	return (sound == Enable) ? true : false;
}

bool Output::getIsFilledChanged() const
{
	return UI.isFillChanged;
}
...
void Output::DrawRect(Point P1, Point P2, GfxInfo RectGfxInfo, bool selected) const
{
	...
	//      to not draw outside the draw area 
...
void Output::DrawSquare(Point p, GfxInfo SquareGfxInfo, bool selected)const
{
	...
	if (p.y > UI.height - UI.StatusBarHeight- HalfsideLenght)
		p.y = UI.height - UI.StatusBarHeight- HalfsideLenght;
...
void Output::DrawHexagon(Point p, GfxInfo HexagonGfxInfo, bool selected)const
{
	...
	if (p.y > UI.height - UI.StatusBarHeight - (sideLenght / 2) * sqrt(3))
		p.y = UI.height - UI.StatusBarHeight - (sideLenght / 2) * sqrt(3);
...
void Output::DrawCircle(Point Pc, Point P, GfxInfo CircleGfxInfo, bool selected)const
{
	...
	if (P.y > UI.height - UI.StatusBarHeight)
		P.y = UI.height - UI.StatusBarHeight;

	int radius = sqrt(pow(Pc.x - P.x, 2) + pow(Pc.y - P.y, 2));
	if (Pc.y < UI.ToolBarHeight + 13 + radius)
		Pc.y = UI.ToolBarHeight + 13 + radius;

	if (Pc.y > (UI.height - UI.StatusBarHeight)-radius)
		Pc.y = (UI.height - UI.StatusBarHeight) - radius;

	if (radius > ((UI.height - (UI.ToolBarHeight + 13 + UI.StatusBarHeight)) / 2.)) //max circle can be drawn
	{
		radius = (UI.height - (UI.ToolBarHeight + 13 + UI.StatusBarHeight)) / 2.;
		Pc.y= ((UI.height+13) / 2.);
	}
