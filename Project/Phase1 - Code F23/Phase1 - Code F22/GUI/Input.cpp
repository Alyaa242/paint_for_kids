...
{	
	...
	else	//GUI is in PLAY mode
	{
		if (y >= 0 && y < UI.ToolBarHeight)
		{
			...
			switch (ClickedItemOrder) {
			case 2*ITM_PICK_BY_FIGURE: return PICK_BY_FIGURE;	//A click on pick by figure icon
			case 2*ITM_PICK_BY_COLOR: return PICK_BY_COLOR;	  //A click on pick by color icon
			case 2*ITM_BOTH: return BOTH;	//A click on both icon
			case 2*ITM_SWITCH_TO_DRAW: return SWITCH_TO_DRAW;	//A click on switch to draw mode icon
			//items were multiblied by 2 to make click action fit with edit in (line 158 in output.cpp)
